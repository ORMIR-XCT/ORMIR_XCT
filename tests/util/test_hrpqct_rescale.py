...
from ormir_xct.core.util.hrpqct_rescale import (
    convert_native_to_linear_attenuation,
    convert_native_to_hu,
    convert_native_to_bmd,
    convert_hu_to_linear_attenuation,
    convert_hu_to_native,
    convert_hu_to_bmd,
    convert_linear_attenuation_to_hu,
    convert_linear_attenuation_to_native,
...
class TestScancoRescale(unittest.TestCase):
    def test_native_to_linear_attenuation(self):
        ...
        mu_scaling = 8192
        result_image = convert_native_to_linear_attenuation(test_image, mu_scaling)
        np.testing.assert_array_equal(expected_image, result_image)

    def test_native_to_hu(self):
        ...
        mu_scaling = 8192
        mu_water = 0.5
        result_image = convert_native_to_hu(test_image, mu_scaling, mu_water)
        np.testing.assert_array_equal(expected_image, result_image)

    def test_native_to_bmd(self):
        # 1. LinearAttenuation = ScancoUnits / mu_scaling
        ...
        rescale_slope = 9
        rescale_intercept = 1
        result_image = convert_native_to_bmd(
    ...
    def test_hu_to_native(self):
        ...
        mu_water = 0.5
        mu_scaling = 8192
        result_image = convert_hu_to_native(test_image, mu_water, mu_scaling)
        np.testing.assert_array_equal(expected_image, result_image)
    ...
    def test_linear_attenuation_to_native(self):
        # ScancoUnits = LinearAttenuation * mu_scaling
        test_image = create_test_image((10, 10, 10), 1)
        expected_image = create_test_image((10, 10, 10), 8192)
        mu_scaling = 8192
        result_image = convert_linear_attenuation_to_native(test_image, mu_scaling)
        np.testing.assert_array_equal(expected_image, result_image)
